...
(() => {
  ...
  function updateAiCheckbox() {
    ...
    box.title = 'Yapay zekâ ile üretilmiş modelleri gizle';
    box.innerHTML =
      '<input id="sf-hide-ai" class="c-checkbox__input" type="checkbox" name="sf-hide-ai">' +
      '<div class="c-checkbox__actor"></div>' +
      '<label tabindex="0" class="c-checkbox__label" for="sf-hide-ai">Hide AI</label>';
    const input = box.querySelector('input');
    input.checked = getHideAI();
    box.classList.toggle('--active', input.checked);
    input.addEventListener('change', () => {
      box.classList.toggle('--active', input.checked);
      try { localStorage.setItem(HIDE_AI_KEY, input.checked ? '1' : '0'); } catch (e) { /* yoksay */ }
    ...
    // "Reset" bağlantısından önce yerleştir
    content.insertBefore(box, content.querySelector('.c-filters__filter.--button'));
  }
